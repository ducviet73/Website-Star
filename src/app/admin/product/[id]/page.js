...
export default function ProductEdit({ params }) {
    ...
    // Fetch categories
    const { data: categories, error: categoriesError } = useSWR("https://star-backend-ragw.onrender.com/categories", fetcher);

    // Fetch product details
    const { data: product, error: productError } = useSWR(productId ? `https://star-backend-ragw.onrender.com/products/detail/${productId}` : null, fetcher);
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values, { resetForm }) => {
            ...
            try {
                const response = await fetch(`https://star-backend-ragw.onrender.com/products/${productId}`, {
                    method: 'PUT',
                    body: data,
                });
                    ...
                    <div className="card rounded-0 border-0 shadow-sm">
                        <div className="card-body">
                            ...
                            <label className="form-label">ảnh sản phẩm cũ</label>
                                <img src={`https://star-backend-ragw.onrender.com/img/${product.image}`}  className="w-50" alt="Current Product" />
                            <div className="mb-3">
                                ...
                                {formik.values.image ? (
                                    ...
                                    <img
                                        src={`https://star-backend-ragw.onrender.com/img/${product.image}`}
                                        className="w-50"
                                        alt="Current Product"
                                    />
                                ) : null}
