...
const UserForm = ({ userId, users, onUserUpdated, onClose }) => {
    ...
    const handleSubmit = (e) => {
        ...
        const request = userId
            ? axios.put(`https://star-backend-z1cm.onrender.com/users/${userId}`, user) // Edit user
            : axios.post('https://star-backend-z1cm.onrender.com/users', user); // Add new user
...
const UserListWithRoleUpdate = () => {
    ...
    const deleteUser = async (id) => {
        if (window.confirm('Bạn có chắc chắn muốn xóa người dùng này?')) {
            try {
                await axios.delete(`https://star-backend-z1cm.onrender.com/users/${id}`);
